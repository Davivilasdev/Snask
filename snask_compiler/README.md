...
**Snask** é uma linguagem de programação **dinâmica** e **interpretada**, projetada para oferecer uma experiência de desenvolvimento **simples** e **poderosa**, combinada com o **alto desempenho** de uma _runtime_ construída em **Rust**.

Seu objetivo é ser a ponte perfeita entre a facilidade de uso de linguagens de _scripting_ e a robustez de sistemas de baixo nível.
...
Um _shell_ profissional com histórico de comandos e comandos especiais para prototipagem rápida.
...
\# Navegue até o diretório do compiladorcd snask\_compiler# Compile o projeto em modo de release para otimizaçãocargo build --release
...
./target/release/snask interpret meu\_programa.snask
...
// Função com tipagem opcionalfun somar(a: float, b: float): float { return a + b;}// Chamada da funçãoprint(somar(10, 20)); // Saída: 30

Controle de Fluxo

O controle de fluxo utiliza a sintaxe if/elif/else e blocos {}.

if x > 10 { print("Maior que 10");} elif x == 10 { print("Igual a 10");} else { print("Menor que 10");}// Loop de iteraçãofor item in \[1, 2, 3\] { print(item);}
...
read\_file(), write\_file(), exists(), delete()

**HTTP & JSON**

Conectividade Web

http\_get( ), json\_parse(), json\_stringify()
...
O comando snask install cuida de todo o processo:
...
\# Exemplo de instalação$ snask install advanced\_math# ... Processo automático de download, integração e compilação ...✅ INSTALAÇÃO COMPLETA!# As funções do módulo 'advanced\_math' estão agora disponíveis globalmente.
...
_Desenvolvido por Davivilasdev_
